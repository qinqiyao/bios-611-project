...
```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',fig.width=3,fig.height=3,message=FALSE,fig.cap="Promotion and Sex"}
library(tidyverse)
library(MASS)
library(dplyr)
library(ggplot2)
...
From Figure 1 which is the four-fold plot between sex and whether an employee promoted, we can see that the proportion of female is smaller than male's. And we can further conclude that male employees are more likely to have promotion chances than female employees.   


```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',message=FALSE,fig.width=6,fig.height=3,fig.cap="Promotion and Recruitment Channel"}
...
Meanwhile, we can find that the proportion of promoted employees who are recruited by others referring is much larger than other two recruitment channels in Figure 2, which indicates that employees with referral recruitment channel have greater chance to be promoted. 


```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',message=FALSE,fig.width=5,fig.height=4,fig.cap="Promotion and Whether won awards"}
...
```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',message=FALSE,fig.width=3,fig.height=3,fig.cap="Promotion and Average Training Score"}
...
```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',message=FALSE,fig.width=5,fig.height=3,fig.cap="Importance of Top 5 Variables"}
library(tidyverse)
library(MASS)
library(dplyr)
library(lime)
...
Firstly, I split data into training and testing sets. Since the data is imbalanced, I used a combination of over-sampling minority examples and under-sampling majority examples with the re-sampling from the rare class probability of 0.5 for training data. After fitting the model by R, we can get Figure 5 showing the most important variables for this model and following accuracy-related statistics based on testing data. Lastly, we can plot ROC curve based on testing data with the AUC of 0.72, which also illustrates that this random forest model works well.
...
```{r,warning=FALSE,comment=NA,echo=FALSE,fig.align='center',message=FALSE,fig.width=8,fig.height=4,fig.cap="LIME Plots"}
library(lime)
fit_rf = readRDS("model/randomforest.rds")
...
![Online Employee Promotion Evaluation System](./pics/shiny2.png)
